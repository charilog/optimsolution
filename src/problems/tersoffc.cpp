    ...
    , m_(3)
{
    ...
    setName("tersoffc");
    setFullName("TersoffC Si(C) Cluster Potential (CEC2011)");
...
void TersoffC::init(int dim) {
    //  dim της μορφής 3N-6, else default N=10 (D=24).
    if (dim >= 3 && (dim + 6) % 3 == 0) {
        int N = (dim + 6) / 3;
        if (N >= 3) {
            num_atoms_ = N;
    ...
    // bounds: x0∈[0,4], x1∈[0,4], x2∈[0,π], x[i≥3]∈[-4.25,4.25]
    Vec lo(D_, -4.25), hi(D_, 4.25);
    if (D_ >= 1) { lo[0] = 0.0;  hi[0] = 4.0;  }
    if (D_ >= 2) { lo[1] = 0.0;  hi[1] = 4.0;  }
    if (D_ >= 3) { lo[2] = 0.0;  hi[2] = PI;   }
    setBounds(lo, hi);
}

double TersoffC::distance(const std::vector<double>& a,
                          const std::vector<double>& b) {
    const double dx = a[0]-b[0], dy = a[1]-b[1], dz = a[2]-b[2];
    return std::sqrt(dx*dx + dy*dy + dz*dz);
}

double TersoffC::angle_abc(const std::vector<double>& a,
                           const std::vector<double>& b,
...
TersoffC::reconstruct_positions(const Vec& x) const {
    // N atoms, 3 coords each
    std::vector<std::vector<double>> pos(num_atoms_, std::vector<double>(3, 0.0));

    // Atom 0 at origin
    pos[0][0]=0.0; pos[0][1]=0.0; pos[0][2]=0.0;

    // Atom 1 on +x axis at distance x0
    if (D_ >= 1) {
        pos[1][0] = x[0]; pos[1][1] = 0.0; pos[1][2] = 0.0;
    }

    // Atom 2 in xy-plane with polar (r=x1, phi=x2)
    if (D_ >= 3) {
        const double r = x[1];
        const double s = std::sin(x[2]), c = std::cos(x[2]);
        pos[2][0] = r * c;
        pos[2][1] = r * s;
        pos[2][2] = 0.0;
    }

    // Atoms 3..N-1: read 3D Cartesians from x[3 + 3*(i-3) .. +2]
...
double TersoffC::VA(double r) const { return B_ * std::exp(-lambda2_ * r); }

double TersoffC::Bij(int i, int j, const std::vector<std::vector<double>>& pos) const {
    ...
    for (int k = 0; k < num_atoms_; ++k) {
        ...
        const double theta = angle_abc(pos[j], pos[i], pos[k]); // θ_ijk at i
...
double TersoffC::evaluate_core(const Vec& x) {
...
void TersoffC::gradient_core(const Vec& x, Vec& g) {
    g.assign(x.size(), 0.0);
