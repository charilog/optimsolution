...
class JAYA : public Optimizer {
public:
    JAYA() = default;
    ~JAYA() override = default;

    std::string methodShortName() const override { return "JAYA"; }
    std::string methodFullName()  const override { return "JAYA (Jaya Optimization Algorithm)"; }
    ...
    void configure(const MethodConfig& mc) override {
        ...
        // Population override from [jaya] (aliases & robust parsing)
        int p = -1;
        ...
        if (p >= 3) {
            pop_override_ = p;
            // base value is updated immediately so it is printed correctly in the header
            this->setPopulation(pop_override_);
        }

        // JAYA option: whether to use |x| in update (as in the original Rao 2016 definition)
        {
            int ua = mc.getInt("use_abs",
                     mc.getInt("useabs",
                     mc.getInt("abs", use_abs_ ? 1 : 0)));
            std::string uas = mc.getStr("use_abs",
                              mc.getStr("useabs",
                              mc.getStr("abs", std::string{})));
            use_abs_ = parse_bool(uas, ua != 0);
        }
        ...
        // Optional echo flag
        debug_jaya_ = mc.getInt("debug_jaya", debug_jaya_);
        debug_jaya_ = parse_int(mc.getStr("debug_jaya", std::to_string(debug_jaya_)), debug_jaya_);
    }
