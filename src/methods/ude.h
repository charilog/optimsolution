 ...
 *
 *   Strategy [3] implements a fuller jSO-style current-to-pbest-w/1 step:
 *     v = x_i + Fw*(x_pbest - x_i) + F*(x_r1 - x_r2)
 *   with success-history adaptation for F and CR, an external archive of
 *   inferior parents, and a dynamic p schedule derived from pbest_p (= pmax).
 *
 ...
 *   pbest_p           jSO pmax fraction (pmin=pmax/2)   (default 0.25)
...
private:
    ...
    Vec mutate_jso  (int i, double Fj, double pj); // jSO/current-to-pbest-w/1 donor

    // Binomial crossover (uses the supplied CR value)
    Vec  crossover  (const Vec& target, const Vec& donor, double CRuse);

    // jSO helpers
    std::pair<double,double> sampleJSOParameters();
    void updateJSOMemory();
    void pushArchive(const Vec& x);
    double weightedLehmerMean(const std::vector<double>& values,
...
private:
    ...
    double F_       = 0.6;
    double CR_      = 0.9;
    double pbest_p_ = 0.25;  // jSO pmax (pmin = pmax / 2)

    // jSO-specific state
    std::vector<Vec>    archive_;      // external archive of inferior parents
    std::vector<double> jso_MF_;       // success-history memory for F
    std::vector<double> jso_MCR_;      // success-history memory for CR
    std::vector<double> jso_SF_;       // successful F values in current generation
    std::vector<double> jso_SCR_;      // successful CR values in current generation
    std::vector<double> jso_dF_;       // corresponding fitness improvements
    int    jso_H_            = 5;      // historical memory size
    int    jso_k_            = 0;      // circular memory index
    double jso_archive_rate_ = 1.0;    // archive size = archive_rate * NP
