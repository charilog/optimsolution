...
// Continuous population-based metaheuristic.
// Implementation follows the canonical EO update using an equilibrium pool
// (four best + their mean) and per-dimension coefficients.
...
public:
    ...
    void configure(const MethodConfig& mc) override {
        ...
        // EO parameters
        a1_ = parse_double(mc.getStr("a1", std::to_string(a1_)), mc.getDbl("a1", a1_));
        a2_ = parse_double(mc.getStr("a2", std::to_string(a2_)), mc.getDbl("a2", a2_));
        gp_ = parse_double(mc.getStr("gp", std::to_string(gp_)), mc.getDbl("gp", gp_));

        if (!std::isfinite(a1_) || a1_ <= 0.0) a1_ = 2.0;
        if (!std::isfinite(a2_) || a2_ <= 0.0) a2_ = 1.0;
        if (!std::isfinite(gp_)) gp_ = 0.5;
        if (gp_ < 0.0) gp_ = 0.0;
        if (gp_ > 1.0) gp_ = 1.0;
        ...
        // Optional echo
        debug_eo_ = mc.getInt("debug_eo", debug_eo_);
        debug_eo_ = parse_int(mc.getStr("debug_eo", std::to_string(debug_eo_)), debug_eo_);
    }
...
private:
    ...
    void buildEquilibriumPool(std::vector<Vec>& pool) const;
