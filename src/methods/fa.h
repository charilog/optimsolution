...
public:
    ...
    void configure(const MethodConfig& mc) override {
        ...
        // Core FA parameters
        beta0_ = parse_double(mc.getStr("beta0", mc.getStr("beta_0", std::to_string(beta0_))), beta0_);
        gamma_ = parse_double(mc.getStr("gamma", std::to_string(gamma_)), gamma_);
        alpha0_ = parse_double(mc.getStr("alpha", std::to_string(alpha0_)), alpha0_);
        alpha_damp_ = parse_double(mc.getStr("alpha_damp", mc.getStr("alpha_decay", std::to_string(alpha_damp_))), alpha_damp_);

        if (!std::isfinite(beta0_) || beta0_ <= 0.0) beta0_ = 1.0;
        if (!std::isfinite(gamma_) || gamma_ < 0.0)  gamma_ = 1.0;
        if (!std::isfinite(alpha0_) || alpha0_ < 0.0) alpha0_ = 0.2;
        if (!std::isfinite(alpha_damp_) || alpha_damp_ <= 0.0) alpha_damp_ = 1.0;
        if (alpha_damp_ > 1.0) alpha_damp_ = 1.0;
        ...
        debug_fa_ = mc.getInt("debug_fa", debug_fa_);
        debug_fa_ = parse_int(mc.getStr("debug_fa", std::to_string(debug_fa_)), debug_fa_);
    }
