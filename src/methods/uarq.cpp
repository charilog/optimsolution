...
void UARQ::configure(const MethodConfig& mc) {
    ...
    // minimum population for LPSR
    int pmin = mc.getInt("popmin",
               mc.getInt("pop_min",
               mc.getInt("minpop",
               mc.getInt("minpopulation",
               mc.getInt("population_min", pop_min_)))));
    if (pmin < 0) pmin = parse_int(mc.getStr("popmin", ""), pop_min_);
    if (pmin >= 2) pop_min_ = pmin;
...
void UARQ::init(){
    ...
    if (debug_arq_) {
        std::fprintf(stdout,
            "[uarq] cfg -> N0=%d, Nmin=%d, p=%.4f, agent=%.3f, muF=%.3f, muCR=%.3f, alpha=%.3f, rho=%.3f, w=%.3f, stag=%d, rtrpool=%d\n",
            pop_init_, pop_min_, pbest_, agent_fraction_, muF_, muCR_, outlier_alpha_, outlier_rho_, worst_frac_, stagnation_trigger_, rtr_pool_);
        std::fflush(stdout);
    }
...
int UARQ::pickDistinct(int n, int a, int b, int c){
...
double UARQ::distBN(const Vec& a, const Vec& b) const {
    const Vec& L = prob_->lb();
    const Vec& U = prob_->ub();
    double s = 0.0;
...
double UARQ::quantile(std::vector<double> v, double q01){
...
void UARQ::archivePush(const Vec& x){
    A_.push_back(x);
}

void UARQ::archiveTrim(int N){
...
void UARQ::sample_F_CR(double& F, double& CR,
...
void UARQ::update_mu_from_success(const std::vector<double>& SF,
...
void UARQ::makeTrial(int i, const std::vector<int>& ord, double F, double CR, Vec& u){
    const int D = prob_->dimension();
...
bool UARQ::selectionRTR(int parentIndex, const Vec& u, double fu,
                        double F, double CR,
    ...
    if (fu < FX_[qstar]) {
        ...
        // log success even for neighbor replacement
        SF.push_back(F);
        SCR.push_back(CR);
        SG.push_back(gain);
        return true;
...
void UARQ::quarantine_and_restart(){
    if (!prob_) return;
...
void UARQ::reducePopulationTo(int newN){
    const int N = (int)X_.size();
    if (newN >= N || newN < 4) return;

    // keep best newN individuals (remove worst)
    std::vector<int> ord(N);
    std::iota(ord.begin(), ord.end(), 0);
    std::sort(ord.begin(), ord.end(), [&](int a, int b){ return FX_[a] < FX_[b]; });

    // push removed to archive (then trimmed)
    for (int k=newN; k<N; ++k){
        archivePush(X_[ord[k]]);
    }
    ...
    for (int k=0; k<newN; ++k){
        X2.push_back(X_[ord[k]]);
        F2.push_back(FX_[ord[k]]);
    }
    ...
    // update best (safe)
    best_f_ = std::numeric_limits<double>::infinity();
    best_x_.assign(prob_->dimension(), 0.0);
    for (int i=0; i<newN; ++i){
        if (FX_[i] < best_f_) { best_f_ = FX_[i]; best_x_ = X_[i]; }
    }

    this->setPopulation(newN);
    archiveTrim(newN);
}

void UARQ::applyLPSR(){
    if (pop_init_ < 4) return;
    if (pop_min_ < 4) pop_min_ = 4;
    if (pop_min_ > pop_init_) pop_min_ = pop_init_;
    ...
    const double N0 = (double)pop_init_;
    const double Nmin = (double)pop_min_;
    int Nt = (int)std::llround(N0 - progress * (N0 - Nmin));
    if (Nt < pop_min_) Nt = pop_min_;
    if (Nt > pop_init_) Nt = pop_init_;

    if (Nt < N) reducePopulationTo(Nt);
}

void UARQ::one_iteration(){
    if (!prob_) return;
    ...
    // linear population size reduction (after maintenance)
    applyLPSR();
    N = (int)X_.size();

    // keep archive bounded to current population
    archiveTrim(N);
