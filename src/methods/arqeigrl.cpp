...
void ARQEigRL::init()
{
    ...
    // More exploration-oriented initialization in zone 0
    ...
    // More exploitation-oriented initialization in zone 1
...
void ARQEigRL::updateMemory(const std::vector<double>& succF,
                            const std::vector<double>& succCR)
{
    ...
    double mF = (denF > 0.0 ? numF / denF : 0.5);

    double meanCR = 0.0;
    int    cntCR  = 0;
    for (double cr : succCR) {
        meanCR += cr;
        ++cntCR;
    }
    double mCR = (cntCR > 0 ? meanCR / (double)cntCR : 0.8);

    MF_[mem_idx_]  = (1.0 - c_mem_) * MF_[mem_idx_]  + c_mem_ * mF;
    MCR_[mem_idx_] = (1.0 - c_mem_) * MCR_[mem_idx_] + c_mem_ * mCR;

    mem_idx_++;
    if (mem_idx_ >= H_) mem_idx_ = 0;
}

int ARQEigRL::determineZone(const Vec& x) const
{
    if (!prob_ || !std::isfinite(best_f_)) return 0;
    double d = bnDistance(x, best_x_);
    if (d <= zone_radius_) return 1; // Close to the best -> exploit
    return 0;                        // Far -> explore
}

int ARQEigRL::selectStrategy(int zone)
{
    if (zone < 0 || zone >= NUM_ZONES_) zone = 0;

    double sumw = 0.0;
    for (int s = 0; s < NUM_STRAT_; ++s) {
        double w = strat_weight_[zone][s];
        if (w < 0.0) w = 0.0;
        sumw += w;
    }
...
void ARQEigRL::recordOutcome(int zone, int strat, double gain)
{
...
void ARQEigRL::updateRL()
{
    for (int z = 0; z < NUM_ZONES_; ++z) {
        ...
        if (maxGain <= 0.0) {
            // No clear preference -> gentle smoothing
            for (int s = 0; s < NUM_STRAT_; ++s) {
...
int ARQEigRL::randomIndexExcept(int n, int forbid)
{
...
int ARQEigRL::pickPBestIndex(double pbest_fraction)
{
    int N = (int)FX_.size();
    if (N <= 0) return 0;
...
void ARQEigRL::make_trial_rand1(int i, Vec& trial, double F, double CR)
{
...
void ARQEigRL::make_trial_pbest(int i, Vec& trial, double F, double CR, double pbest_fraction)
{
    const int N = (int)X_.size();
    const Vec& xi = X_[i];
...
void ARQEigRL::make_trial_best1(int i, Vec& trial, double F, double CR)
{
    const Vec& xi = X_[i];
...
void ARQEigRL::make_trial_gaussian(Vec& trial, double sigma)
{
    if (!prob_) return;
    const auto& L = prob_->lb();
    const auto& U = prob_->ub();
...
void ARQEigRL::make_trial_longjump(Vec& trial)
{
    if (!prob_) return;
    const auto& L = prob_->lb();
    const auto& U = prob_->ub();
    ...
    // Randomly selects whether the long jump is global or around a promising point
    bool around_best = (U01_(rng_) < 0.5);
    Vec center = best_x_;
    if (!std::isfinite(best_f_) || !around_best) {
        center.assign(D_, 0.0);
        for (int j = 0; j < D_; ++j) {
            center[j] = L[j] + U01_(rng_) * (U[j] - L[j]);
        }
    }

    double big_sigma = 0.6;
    for (int j = 0; j < D_; ++j) {
        double step = big_sigma * (U[j] - L[j]) * N0(rng_);
        trial[j] = center[j] + step;
    }
}

void ARQEigRL::restartIndividuals()
{
    if (!prob_) return;
    int N = (int)X_.size();
    if (N <= 4) return;
...
void ARQEigRL::one_iteration()
{
    ...
    for (int i = 0; i < N; ++i) {
        ...
        if (ftrial < FX_[i]) {
            ...
            X_[i]  = std::move(trial);
            FX_[i] = ftrial;
            if (archive_rate_ > 0.0) pushArchive(old);
    ...
    // In-run local search (if enabled in the settings)
