...
class TLBO : public Optimizer {
public:
    TLBO() = default;
    ~TLBO() override = default;

    std::string methodShortName() const override { return "TLBO"; }
    std::string methodFullName()  const override { return "Teaching-Learning-Based Optimization (TLBO)"; }
    ...
    void configure(const MethodConfig& mc) override {
        ...
        // Population override from [tlbo] (aliases)
        int p = -1;
        ...
        // Optional fixed teaching factor (1 or 2). Default: 0 (random per learner)
        int tf = mc.getInt("teaching_factor",
                 mc.getInt("TF",
                 mc.getInt("tf", teaching_factor_fixed_)));
        if (tf <= 0) tf = parse_int(mc.getStr("teaching_factor", ""), tf);
        if (tf <= 0) tf = parse_int(mc.getStr("TF", ""), tf);
        if (tf <= 0) tf = parse_int(mc.getStr("tf", ""), tf);
        if (tf == 1 || tf == 2) teaching_factor_fixed_ = tf;
        else teaching_factor_fixed_ = 0;
        ...
        // Optional echo flag
        debug_tlbo_ = mc.getInt("debug_tlbo", debug_tlbo_);
        debug_tlbo_ = parse_int(mc.getStr("debug_tlbo", std::to_string(debug_tlbo_)), debug_tlbo_);
    }
...
private:
    ...
    int         teaching_factor_fixed_ = 0; // 0 -> random {1,2}
};
