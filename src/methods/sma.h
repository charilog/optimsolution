...
public:
    ...
    void configure(const MethodConfig& mc) override {
        ...
        // Population override from [sma] (aliases)
        int p = -1;
        ...
        // SMA parameters
        z_ = parse_double(mc.getStr("z", std::to_string(z_)), mc.getDbl("z", z_));
        if (z_ < 0.0) z_ = 0.0;
        if (z_ > 1.0) z_ = 1.0;

        a_cap_ = parse_double(mc.getStr("a_cap", std::to_string(a_cap_)), mc.getDbl("a_cap", a_cap_));
        if (!std::isfinite(a_cap_) || a_cap_ <= 0.1) a_cap_ = 5.0;
        ...
        debug_sma_ = mc.getInt("debug_sma", debug_sma_);
        debug_sma_ = parse_int(mc.getStr("debug_sma", std::to_string(debug_sma_)), debug_sma_);
    }
...
private:
    ...
    double      z_ = 0.03;    // random re-initialization probability
    double      a_cap_ = 5.0; // safety cap for atanh-based a
