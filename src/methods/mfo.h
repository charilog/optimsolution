...
public:
    ...
    void configure(const MethodConfig& mc) override {
        ...
        // spiral constant b
        b_ = mc.getDbl("b", b_);
        b_ = parse_double(mc.getStr("b", std::to_string(b_)), b_);
        if (!std::isfinite(b_) || b_ <= 0.0) b_ = 1.0;
        ...
        debug_mfo_ = mc.getInt("debug_mfo", debug_mfo_);
        debug_mfo_ = parse_int(mc.getStr("debug_mfo", std::to_string(debug_mfo_)), debug_mfo_);
    }
