...
private:
    ...
    // Adaptive weighted current-to-pBest controller.
    bool   adaptive_weight_enable_{true};
    double fw_min_mul_{0.45};
    double fw_max_mul_{1.85};
    double fw_abs_max_{1.50};
    double fw_target_success_{0.18};
    double success_fast_ema_{0.0};
    double success_slow_ema_{0.0};
    double good_gain_fast_ema_{1e-6};
    double good_gain_slow_ema_{1e-6};
    int    no_best_improve_iters_{0};
    int    iteration_counter_{0};

    // Predictive prescreen driven by recent successful moves.
    bool   predictive_prescreen_enable_{false};
    double predictive_prescreen_start_{0.10};
    double predictive_prescreen_percent_{0.35};
    double predictive_prescreen_step_floor_{0.010};
    ...
    double good_step_fast_ema_{0.020};
    double good_step_slow_ema_{0.020};
    double good_proxy_fast_ema_{0.35};
    double good_proxy_slow_ema_{0.35};
    ...
    static double relativeImprovement(double old_f, double new_f);
    ...
    double candidateProxyScore(const Vec& xi,
                               const Vec& ui,
                               const Vec& xp,
                               const Vec& xr1,
                               const Vec& xr2,
                               int rank_pos,
                               int N) const;
    double dynamicPrescreenPercent(double progress) const;
