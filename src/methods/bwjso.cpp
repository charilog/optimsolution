...
void BWJSO::configure(const MethodConfig& mc)
{
    // Base: current global population (from [global]).
    int basePop = population();
    if (basePop < 4) basePop = 4;

    // If a population is provided in [bwjso], it takes precedence.
    ...
    // New optional controls for the best-worst correction.
    bw_enable_ = parse_bool_like(mc.getStr("bw_enable", bw_enable_ ? "1" : "0"), bw_enable_);

    bw_group_fraction_ = mc.getDbl("bw_group_fraction", bw_group_fraction_);
    if (bw_group_fraction_ == 0.0) bw_group_fraction_ = -1.0;
    if (bw_group_fraction_ > 1.0) bw_group_fraction_ = 1.0;

    bw_best_alpha_ = mc.getDbl("bw_best_alpha", bw_best_alpha_);
    if (bw_best_alpha_ < 0.0) bw_best_alpha_ = 0.0;
    if (bw_best_alpha_ > 10.0) bw_best_alpha_ = 10.0;

    bw_worst_beta_ = mc.getDbl("bw_worst_beta", bw_worst_beta_);
    if (bw_worst_beta_ < 0.0) bw_worst_beta_ = 0.0;
    if (bw_worst_beta_ > 10.0) bw_worst_beta_ = 10.0;
}
...
void BWJSO::ensureInBounds(Vec& x)
{
...
bool BWJSO::isInBounds(const Vec& x) const
{
...
void BWJSO::trimArchive(int max_size)
{
...
Vec BWJSO::meanOfBest(const std::vector<int>& idx_sorted, int count) const
{
    const int D = prob_->dimension();
    Vec mean(D, 0.0);
    if (count <= 0 || idx_sorted.empty()) return mean;
    if (count > (int)idx_sorted.size()) count = (int)idx_sorted.size();
...
Vec BWJSO::meanOfWorst(const std::vector<int>& idx_sorted, int count) const
{
    const int D = prob_->dimension();
    Vec mean(D, 0.0);
    if (count <= 0 || idx_sorted.empty()) return mean;
    ...
    const int N = (int)idx_sorted.size();
    for (int k = 0; k < count; ++k) {
        const Vec& x = X_[idx_sorted[N - 1 - k]];
        for (int j = 0; j < D; ++j) mean[j] += x[j];
    }
    const double inv = 1.0 / (double)count;
    for (int j = 0; j < D; ++j) mean[j] *= inv;
...
void BWJSO::one_iteration()
{
    ...
    // Best/worst means are computed deterministically and consume no RNG.
    int bw_group_size = p_best_size;
    if (bw_group_fraction_ > 0.0) {
        bw_group_size = static_cast<int>(std::round(bw_group_fraction_ * N));
    }
    if (bw_group_size < 2) bw_group_size = 2;
    if (bw_group_size > N) bw_group_size = N;

    Vec xbests;
    Vec xworsts;
    if (bw_enable_) {
        xbests = meanOfBest(idx_sorted, bw_group_size);
        xworsts = meanOfWorst(idx_sorted, bw_group_size);
    }
        ...
        for (int repeat = 0; repeat < 64 && !feasible; ++repeat) {
            ...
            for (int j = 0; j < D; ++j) {
                if (U01(rng_) < CRi || j == jrand) {
                    ...
                    if (bw_enable_) {
                        donor += (bw_best_alpha_  * Fw) * (xbests[j]  - xi[j]);
                        donor -= (bw_worst_beta_ * Fw) * (xworsts[j] - xi[j]);
                    }
    ...
    // Intentionally kept identical to the uploaded original jSO.
    if (!SF.empty()) {
        double sum_dF = 0.0;
